...
\justifying 
Nel campo sono presenti l'header IP più 8 byte per una lunghezza 
complessiva di 28 byte.
%si potranno usare varie coppie nelle quali inserire i dati. 
\vspace{1ex} \newline 
Nel nostro caso si inserirà un pacchetto ICMP/IP; sebbene si possono 
usare anche altre combinazioni di protocolli (e.g TCP/IP).
%In esso si potranno usare varie tipologie di pacchetti purchè la lunghezza non superi i 28 byte.
Inoltre siccome non rappresenta un datagram da inviare ma già spedito, 
si possono sfruttare un maggior numero di campi. 
\column{0.48\textwidth} 
...
\column{0.48\textwidth}   
\justifying 
Dalle specifiche RFC (792 e 4434) il campo deve essere 0. Quindi Scapy, per 
inviarlo conforme allo standard, azzererà qualsiasi valore al suo interno. 
Il problema può essere risolto tramite degli accorgimenti.  
Tramite la libreria \textbf{struct} è possibile costruire un pacchetto 
avente i dati all'interno del campo e si usera Scapy solo per l'invio. 
...
\includegraphics[width=0.7\textwidth]{../Tesi/img/ICMP_storageCC/immagine_parameterproblem_unused.png}
\captionof{figure}{Messaggio ICMP con campo \textit{unused} impostato}     
\end{columns} 
\vspace{1ex} 
Siccome il suo utilizzo rende il messaggio non conforme agli standard RFC, 
...
%\begin{frame}
%\frametitle{Inserimento dei dati nel campo Pointer} 
%\begin{columns}[T,onlytextwidth]
%\column{0.48\textwidth}   
%\justifying 
%Indica l'ottetto del messaggio in cui è presente l'errore; tuttavia può 
%contenere dei dati non correlati ad esso. 
...
Un alternqativa è l'inserire il dato in tutto il campo ma, rappresentando 
un valore temporale, cio porterebbe al rischio di avere tempi errati o non 
congruenti. Per esempio nel pacchetto viene risultare ricevuto prima che 
sia stato inviato. 
\column{0.48\textwidth} 
%\begin{figure}
