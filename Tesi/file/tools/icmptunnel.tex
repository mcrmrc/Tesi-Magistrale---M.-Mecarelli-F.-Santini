...
\end{enumerate}
\begin{enumerate}
    \item On the client side, get out your gateway and the corresponding interface: route -n
    Edit client.sh and replace  $<\text{server}>$ with the IP address of the proxy server. $<\text{gateway}>$ with gateway address obtained above and similarly for $<\text{interface}>$.
\end{enumerate}
\begin{enumerate}
    ...
    \item Run the tunnel on your client with root privileges: [sudo] ./icmptunnel -c $<\text{server}>$ 
\end{enumerate}
...
\subsection*{icmptunnel-docker-demo}
