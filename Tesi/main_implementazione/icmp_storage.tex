...
\begin{minipage}{\textwidth}
    \centering 
    \captionof{figure}{Messaggio IP/ICMP con campo \textit{unused} azzerato \cite{scapy_pdfdump}} 
...
pacchetto evitando di farlo creare da Scapy. 
...
\begin{minipage}{\textwidth} 
    \centering 
    \captionof{figure}{Messaggio IP/ICMP con campo \textit{unused} impostato \cite{scapy_pdfdump}} 
...
Si potrà quindi scegliere se inserire dati nel campo o no.   
...
\begin{minipage}{\textwidth}
    \centering
    \captionof{figure}{Messaggio IP/ICMP con campo \textit{Header+64 bit} impostato \cite{scapy_pdfdump}} 
    \label{fig:covertStorage:header64bit} 
    %\includegraphics[width=0.5\textwidth]{./img/ICMP_storageCC/immagie_header_64bit.png} 
    \includegraphics[width=0.8\textwidth]{./img/ICMP_storageCC/packet_destination_unreachable_page_1.png}
\end{minipage} 
\begin{center}
...
\end{lstlisting}
\captionof{figure}{Dump esadecimale del messaggio in figura \ref{fig:covertStorage:header64bit}}
...
\begin{minipage}{\textwidth}
    \centering 
    \captionof{figure}{Messaggio IP/ICMP con campo \textit{pointer} impostato \cite{scapy_pdfdump}} 
...
\end{lstlisting}
\captionof{figure}{Dump esadecimale del messaggio in Figura \ref{fig:covertStorage:pointer}}
...
Se il suo valore cambiasse troppo spesso la cosa potrebbe risultare sospetta 
(a differenza del campo \textit{Identifier}). 
...
\begin{minipage}{\textwidth} 
    \centering 
    \captionof{figure}{Messaggio IP/ICMP con campo \textit{identifier} impostato \cite{scapy_pdfdump}} 
...
\end{lstlisting}
\captionof{figure}{Dump esadecimale del messaggio in Figura \ref{fig:covertStorage:identifier}}
...
\begin{minipage}{\textwidth}
    \centering 
    \captionof{figure}{Messaggio IP/ICMP con campo \textit{Data} impostato \cite{scapy_pdfdump}} 
...
\end{lstlisting}
\captionof{figure}{Dump esadecimale del messaggio in Figura \ref{fig:covertStorage:payload}}
...
La Figura \ref{fig:covertStorage:timestamp:output} mostra come in ciascun 
campo timestamp verrà inserito un byte nella parte 
dei millisecondi come si può vedere nella 
Figura \ref{fig:covertStorage:timestamp:output}. 
In essa viene indicato il momento dell'inserimeno del carattere 
'r' che nella codifica UNICODE appartiene all'intero 114. 
Successivamente si illustrano le modifiche effettuate al campo 
timestamp e si può vedere che il dato relativo al carattere 
nascoso rimane identificabile nella parte dei millisecondi. 
\newline
\begin{lstlisting}
...
\end{lstlisting}
\captionof{figure}{Esempio di un dato inserito nel campo Timestamp}
...
\vspace{1ex} \newline
Si potrebbero inserire il dato in tutto il campo rappresentante il dato temporale; 
ma questo ci espone al rischio di avere tempi errati\footnote{
    Per esempio il pacchetto risulta ricevuto prima che sia stato inviato. 
}. 
\vspace{2ex} \newline 
...
\begin{minipage}{\textwidth}
    \centering 
    \captionof{figure}{Messaggio IP/ICMP con campi \textit{Timestamp} impostati \cite{scapy_pdfdump}} 
...
%\end{lstlisting}
%\captionof{figure}{Dump esadecimale del messaggio in Figura \ref{fig:covertStorage:time}}
%\label{code:covertStorage:data} 
%\end{minipage}
%\end{center}
