from Programma.methods.check_type import is_namespace, is_ipaddress, is_integer, is_enum_member, is_boolean, is_string, is_list, is_bytes, is_socket
from Programma.methods.get_methods import get_threading_Lock
from Programma.methods.network_methods import firewall_disable, firewall_enable, insert_ip_host, get_local_IP
...
from Programma.main_entry import Attacker, Victim, Proxy
import ipaddress, os, json, socket, threading

class Attacker_Methods(): 
    def __init__(self, attaccante:Attacker=None):
        if not isinstance(attaccante, Attacker):
            raise TypeError("Attacker_Methods: attaccante non istanza di Attacker")
...
class Proxy_Methods: 
    def __init__(self, proxy:Proxy=None):
        if not isinstance(proxy, Proxy):
            raise TypeError("Proxy_Methods: proxy non istanza di Proxy")
        self.proxy = proxy
...
class Victim_Methods: 
    def __init__(self, victim:Victim=None):
        if not isinstance(victim, Victim):
            raise TypeError("Victim_Methods: victim non istanza di Victim")
        self.victim = victim
