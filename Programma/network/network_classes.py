from type.check_type import is_ipaddress, is_AsyncSniffer, is_threading_Timer, is_threading_Event, is_integer, is_bytes, is_callable_function, is_time
from type.get_type import get_timer, get_AsyncSniffer, get_threading_Lock, get_threading_Event
from classes import CALC 
from network.network_methods import find_local_IP, find_public_IP, get_IPv6_scopeID, interface_from_IPAddress, check_sniffer_args
from scapy.all import AsyncSniffer, Ether, IP, ICMP, sendp 
from utils_methods import threadEvent_set
import ipaddress, sys, threading, random

class HOST_IPs: 
    def __init__(self):
        local_IP, error=find_local_IP() 
        if error: 
            raise Exception("Errore nella rilevazione dell'IP privato: ",error)
        self.local_IP=ipaddress.ip_address(local_IP)
        self.global_IP=ipaddress.ip_address(find_public_IP())
