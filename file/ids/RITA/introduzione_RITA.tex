...
Una costante assoluta su cui noi possiamo fare affidamento per rilevare i malware, è che dovranno chiamare "casa". 
Da questo presupposto; analizzando il traffico di rete, possiamo rilevare le chiamate C2 indipendentemente dalla 
piattaforma. %e senza la necessità di agenti endpoint.
...
Per una maggiore fedeltà, l'analisi del comportamento e dei pattern presneit, deve analizzare la comunicazione 
...
La persistenza è l'attributo chiave che ricerchiamo quando cerchiamo sistemi compromessi. 
...
Durante la fase di analisi, si suddividono questi indicatori principali di persistenza in moduli separati. 
...
RITA acquisisce i log di connessione di Zeek e tramite l'utilizzo dell'analisi comportamentale identifica i sistemi potenzialmente compromessi.
%It ingests Zeek connection logs and uses behavioral analytics to identify potentially compromised systems.
\vspace{2ex} \newline  
La gravità di ciascun beacon è determinata da quattro fattori. 
...
\begin{longtable}{|p{0.25\textwidth}|p{0.55\textwidth}|} 
    ...
    \hline
    Beacons &  
    I 'Beacon' sono impulsi di comunicazioni fra due host. 
...
%    \hline  
    ...
    \\
    \hline 
\caption{Moduli principali di RITA per l'analisi} 
...
Inoltre la maggior parte dei moduli ha soglie che possono essere modificate tramite il file di configurazione. %di RITA.
