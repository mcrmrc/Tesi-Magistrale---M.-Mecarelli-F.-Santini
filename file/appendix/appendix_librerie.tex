...
\end{lstlisting}
\captionof{lstlisting}{Dizionario con le yipologi di attacco utilizzabili} 
...
\subsection{Scegliendo la tipologia di attacco}
...
La classe conterrà i metodi utilizzati per ricevere i dati mandati dall'identità (che nel nostro caso saranno la vittima, il proxy o in caso l'attaccnate). 
In questa classe saranno quindi definiti, per ogni tipologia di attacco sfruttabile [Code:\ref{script:appendice:attacksingleton:attackDict}], 
i metodi che permetteranno di ricevere i dati inviati. 

\subsection*{IPv4 Information Request} 
...
\begin{itemize}
    ...
    \item Un threading Evetn, utilizzato per indicare quando si è finito di monitorare il traffico di rete. 
...
\begin{lstlisting}[
    ...
    ,literate={.}{{\char46}}1 {:}{{\char58}}1 {`}{{\texttt{\char96}}}1 {"}{{\texttt{"}}}1 
] 
...
\subsection*{Metodi successivi (che usano IPv4)} 
...
\subsection*{Metodi successivi (che usano IPv6)} 
Nel caso di questi metodi, che usano IPv6, le modifiche saranno simili a quelle dei metodi precedenti. 
Tuttavia in questo caso nel filtro nonm si userà il protocollo ICMP ma il protocollo ICMP6. 
\vspace{2ex} 
\begin{lstlisting}[
    language=Python
    ,basicstyle=\footnotesize
...
\subsection*{Callback usate dai metodi che usano IPv4} 
...
def callback_v4_timing_cc(timer_func, event,timer,data,prev_time=None, num_bit=0 ): 
    ...
    return callback
\end{lstlisting}
\captionof{lstlisting}{Callback del metodo \textbf{v4\_timing\_cc}} 
...
\subsection*{Callback usate dai metodi che usano IPv6} 
Gli argomenti in input, richiesti dal metodo, sono: 
\begin{itemize}
    \item Una funzione di callback. questa verrà poi usata per impostare il timer [Code:\ref{script:appendice:attacksingleton:waitData:callback:v6_timing_cc}  line 19] 
    \item Un threading Event; usato per segnalare quando il metodo termina. 
...
def cb_v6_timing_cc(timer_func, event, timer, data, prev_time=None, num_bit=0):   
    ...
    return callback
\end{lstlisting}
\captionof{lstlisting}{Callback del metodo \textbf{v6\_timing\_cc}} 
...
\subsection{Aspettando i dati}
Quando si volgiono ricevere dei dati tramite gli attacchi metodi definiti prima; 
...
\subsection*{Metodi che usano IPv4}
...
\end{lstlisting}
\captionof{lstlisting}{Metodo che usa \textit{ParamProblem}} 
...
def ipv4_destination_unreachable(data:bytes=None, ip_dst): 
    ...
    return False
\end{lstlisting}
\captionof{lstlisting}{Metodo che usa \textit{DestUnreachable}} 
...
\end{lstlisting}
\captionof{lstlisting}{Metodo che definisce un \textit{Timing Covert Channel}} 
...
def ipv4_timing_channel_2bit(data:bytes=None, ip_dst): 
    ...
    end_time=datetime.datetime.now(datetime.timezone.utc) 
\end{lstlisting}
\captionof{lstlisting}{Metodo che usa \textit{ }} 
...
\subsection*{Metodi che usano IPv6}
...
def ipv6_information_reply(data:bytes=None, addr_src, addr_dst): 
    ...
    return False 
\end{lstlisting}
\captionof{lstlisting}{Metodo che usa \textit{v6\_information\_reply}} 
...
def ipv6_time_exceeded(data:bytes=None, addr_src, addr_dst): 
        ...
        return False
\end{lstlisting}
\captionof{lstlisting}{Metodo che usa \textit{v6\_time\_exceeded}} 
...
def ipv6_packet_to_big(data:bytes=None, addr_src, addr_dst): 
        ...
        return False
\end{lstlisting}
\captionof{lstlisting}{Metodo che usa \textit{v6\_packet\_to\_big}} 
...
def ipv6_destination_unreachable(data:bytes=None, addr_src, addr_dst): 
        ...
        for index in range(0, len(data), 4): 
            dummy_pkt=(
                IPv6(dst=f"{addr_dst}%{interface}",src=addr_src, plen=int.from_bytes(data[index:index+2]))  /
        ...
        return False
\end{lstlisting}
\captionof{lstlisting}{Metodo che usa \textit{v6\_destination\_unreachable}} 
...
def ipv6_timing_channel_1bit(data:bytes=None, addr_src, addr_dst): 
        ...
        end_time=datetime.datetime.now(datetime.timezone.utc) 
\end{lstlisting}
\captionof{lstlisting}{Metodo che usa \textit{v6\_timing\_channel\_1bit}} 
...
def ipv6_timing_channel_2bit(data:bytes=None, addr_src, addr_dst): 
        ...
        end_time=datetime.datetime.now(datetime.timezone.utc) 
\end{lstlisting}
\captionof{lstlisting}{Metodo che usa \textit{v6\_timing\_channel\_2bit}} 
\label{script:appendice:attacksingleton:send:v6_timing_channel:2bit} 

\subsubsection*{Timing Covert Channel con 4 bit} 
Un \textit{Timing Covert Channel} a \textbf{4 bit} sarà simile a quello a \textit{due bit} [Code:\ref{script:appendice:attacksingleton:send:v6_timing_channel:2bit}]. 
Ciò che cambierà è il numero di bit che vengono estratti (in questo caso 4). 
\vspace{1ex} \newline 
Tuttavia sebbene un numero maggiore di bit vengono codificati anche il tempo necessario per poterli spedire aumenta. 
In particolare si sono testati i vari \textit{Timing channel} sull testo \textbf{echo 'Ciao'}. 
...
def ipv6_timing_channel_4bit(self, data:bytes=None, addr_src:ipaddress.IPv6Address=None, addr_dst:ipaddress.IPv6Address=None): #Exec Time 0:22:20.745110 
        ...
        end_time=datetime.datetime.now(datetime.timezone.utc) 
\end{lstlisting}
\captionof{lstlisting}{Metodo che usa \textit{v6\_timing\_channel\_4bit}} 
\label{script:appendice:attacksingleton:send:v6_timing_channel:4bit} 
...
def send_data(attack_func, data:bytes=None, ip_dst):
    ...
    return None
\end{lstlisting}
\captionof{lstlisting}{Inviando i dati} 
\label{script:appendice:attacksingleton:sendData} 


\subsection{Aspetta i dati dalla vittima}
...
\end{lstlisting}
\captionof{lstlisting}{Aspettando i dati dalla vittima} 
\label{script:appendice:attacksingleton:waitData} 
