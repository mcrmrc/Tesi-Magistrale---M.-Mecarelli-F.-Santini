...
per la diagnostica, la segnalazione di errori, per le informazioni di controllo e la 
...
può essere utilizzato in modo improprio per degli attacchi. 
...
\end{minipage} 
%
\subsection{Utilizzato ICMP nelle reti} 
...
%\end{itemize}
\subsection{Rischi per la sicurezza di ICMP e mitigazioni} 
ICMP è un protocollo fondamentale per la diagnostica di rete e la segnalazione di errori, ma può anche essere sfruttato per vari attacchi informatici o per la ricognizione della rete (network reconnaissance). 
Gli aggressori utilizzano ICMP per attacchi DDoS, di ricognizione, di esfiltrazione di dati e covert channel.
%
\subsection*{Attacchi Denial-of-Service (DoS/DDoS) basati su ICMP}
...
\end{itemize}
%
\subsection*{Attacchi di ricognizione basati su ICMP}
...
ICMP è un protocollo fondamentale per la diagnostica di rete, la segnalazione degli errori 
e il controllo delle comunicazioni sia in IPv4 che in IPv6.
Tuttavia, può essere sfruttato per gli attacchi, quindi è necessario implementare misure 
di sicurezza come il filtraggio dei firewall, la limitazione della velocità e il 
rilevamento delle anomalie.  
\subsubsection*{Buone pratiche di sicurezza per ICMP}
...
%
\subsection{Buona practice di sicurezza per ICMP}
ICMP è essenziale per la diagnostica di rete, ma è anche un bersaglio per attacchi DDoS, di ricognizione, 
Covert Channel e di esfiltrazione dati.
\vspace{1ex} \newline 
Limitando l'utilizzo di ICMP, implementando firewall, monitorando il traffico e utilizzando strumenti di 
sicurezza, le organizzazioni possono proteggere le proprie reti dalle minacce basate su ICMP. 
\vspace{2ex} \newline
...
\begin{itemize}
    \item Utilizzare i sistemi di rilevamento delle intrusioni (IDS/IPS) per rilevare gli abusi ICMP.
